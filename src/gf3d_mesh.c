...
VkVertexInputAttributeDescription* gf3d_mesh_get_attribute_descriptions(Uint32 *count) {
    mesh_manager.attributeDescriptions[0] = (VkVertexInputAttributeDescription){0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, vertex)};
    mesh_manager.attributeDescriptions[1] = (VkVertexInputAttributeDescription){0, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, normal)};
    mesh_manager.attributeDescriptions[2] = (VkVertexInputAttributeDescription){0, 2, VK_FORMAT_R32G32_SFLOAT, offsetof(Vertex, texel)};
...
void gf3d_mesh_primitive_create_vertex_buffers(MeshPrimitive *prim) {
    ...
    VkDeviceSize bufferSize = sizeof(Vertex) * vcount;
...
Mesh* gf3d_mesh_load_obj(const char *filename) {
    ...
    mesh = gf3d_mesh_new();
    if (!mesh) {
        slog("failed to allocate mesh for %s", filename);
        gf3d_obj_free(obj);
        return NULL;
    }

    gfc_line_cpy(mesh->filename, filename);
    slog("Loaded mesh filename: %s", mesh->filename);

    primitive = gf3d_mesh_primitive_new();
    if (!primitive) {
        slog("Failed to allocate primitive for %s", filename);
        gf3d_obj_free(obj);
        gf3d_mesh_free(mesh);
        return NULL;
    }
